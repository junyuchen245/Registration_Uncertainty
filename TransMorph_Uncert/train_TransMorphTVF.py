...
def main():
    ...
    save_dir = 'TransMorphTVF_ncc_{}_diffusion_{}_dsc_{}/'.format(weights[0], weights[1], weights[2])
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            loss_dsc = criterion_dsc(def_seg, y_seg.long()) * weights[2]
            ...
            loss_dsc = criterion_dsc(def_seg, x_seg.long()) * weights[2]
            loss = loss_ncc + loss_reg + loss_dsc
            ...
            print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Seg Dice: {:.6f}, Reg: {:.6f}'.format(idx, len(train_loader),
                                                                                            loss.item(),
                                                                                            loss_ncc.item(),
                                                                                            loss_dsc.item(),
                                                                                            loss_reg.item()))
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                x_seg_oh = torch.squeeze(x_seg_oh, 1)
                x_seg_oh = x_seg_oh.permute(0, 4, 1, 2, 3).contiguous()
                def_seg = spatial_trans(x_seg_oh.cuda().float(), flow.cuda())
                def_seg = torch.argmax(def_seg, dim=1, keepdim=True)
                def_out = spatial_trans(x.cuda().float(), flow.cuda())
