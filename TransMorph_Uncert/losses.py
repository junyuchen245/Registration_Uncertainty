...
class DiceLoss(nn.Module):
    ...
    def __init__(self, num_class=4, if_onehot=True):
        super().__init__()
...
class NLLLoss(nn.Module):
    ...
    def __init__(self, reduction: str = 'mean', if_onehot=True, num_class=4):
        super(NLLLoss, self).__init__()
        self.reduction = reduction
        self.if_onehot = if_onehot
        self.num_class = num_class

    def forward(self, inputs, target, sigma):
        if self.if_onehot:
            target = nn.functional.one_hot(target, num_classes=self.num_class)
            target = torch.squeeze(target, 1)
            target = target.permute(0, 4, 1, 2, 3).contiguous()
        mu = inputs
        neg_logvar = torch.clamp(sigma, min=-20, max=20)

        loss = torch.exp(neg_logvar) * torch.abs(target - mu) - neg_logvar
...
class BetaNLLLoss(nn.Module):
    def __init__(self, reduction: str = 'mean', if_onehot=True, num_class=4, beta=1):
    ...
    def forward(self, inputs, target, sigma):
        ...
        mu = inputs
        neg_logvar = torch.clamp(sigma, min=-20, max=20)
        nlogvar = neg_logvar.clone().detach()
        ll = torch.exp(neg_logvar) * torch.pow(target - mu, 2) - neg_logvar
        weight = torch.exp(-nlogvar).detach() ** self.beta
        loss = ll*weight
        if self.reduction == 'mean':
            return loss.mean()
        elif self.reduction == 'sum':
...
class NCC_vxm(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        # set window size
        win = [5] * ndims if self.win is None else self.win #9
