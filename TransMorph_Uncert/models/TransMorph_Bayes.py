...
import numpy as np
import models.configs_TransMorph_Bayes as configs
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., drop=0.):
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, drop=MC_drop)
...
class UncertLayer(nn.Module):
    def __init__(self, num_chan=4, embed_dim=96):
        super(UncertLayer, self).__init__()
        self.conv1 = Conv3dReLU(1+embed_dim//2, embed_dim//2, 3, 1, use_batchnorm=True)
        self.conv2 = Conv3dReLU(embed_dim//2, embed_dim, 3, 1, use_batchnorm=True)
        self.conv3 = Conv3dReLU(embed_dim, embed_dim//2, 3, 1, use_batchnorm=True)
        self.out = nn.Conv3d(embed_dim//2, num_chan, kernel_size=3, padding=1, bias=False)
        self.out.weight = nn.Parameter(Normal(0, 1e-5).sample(self.out.weight.shape))
...
class TransMorphTVFUncert(nn.Module):
    def __init__(self, config, time_steps=7, SVF=False):
        '''
        TransMorph-TVF Uncertainty Model

        1. Chen, Junyu, et al.
        "Transmorph: Transformer for unsupervised medical image registration." Medical image analysis 82 (2022): 102615.

        2. Chen, Junyu, et al.
        "From Registration Uncertainty to Segmentation Uncertainty." IEEE ISBI (2024).
        '''
        super(TransMorphTVFUncert, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        for t in range(self.time_steps):
            ...
            self.up3s.append(DecoderBlock(embed_dim//2, config.reg_head_chan, skip_channels=embed_dim//2 if if_convskip else 0,
                                   use_batchnorm=False))
        self.spatial_trans_half = SpatialTransformer((config.img_size[0]//2, config.img_size[1]//2, config.img_size[2]//2))
...
CONFIGS = {
    'TransMorph-3-LVL-Bayes': configs.get_3DTransMorph3Lvl_config(),
